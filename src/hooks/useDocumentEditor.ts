...
import { remapColumnsForNewSchema } from '../engine/config';
...
const EMPTY_CONFIG: ReportConfig = {
  ...
  design: { fileName: 'extracted_data', showSummary: true, density: 'comfortable' }
};
...
export function useDocumentEditor(initial: DocumentEditorInitial = {}) {
  ...
  const initialDocument = initial.document ?? EMPTY_SESSION;
  ...
  const report = useMemo(() => processReport(raw, config), [raw, config]);
  const hasDataChanges = useMemo(() => {
    const baseline = originalSessionRef.current;
    return !!baseline && JSON.stringify(session.state) !== JSON.stringify(baseline);
  }, [session.state]);
  ...
  return { session, raw, config, report, hasDataChanges, updateConfig, updateRaw, evaluateTypeChange, commitTypeChange, applyColumnChanges, validateImportColumnData, handleImportColumnData, handleHeaderRowChange, handleEditCell, handleAddRow, handleAddColumn, handleDeleteColumn, handleCopyCell, handlePasteCell, getColumnImportTargets, handleAddCell, handleDeleteCell, handleInsertCellBoundary, handleRemoveRows, handleCopyRows, handleCutRows, handlePasteRows, handleImportRows, resetToBaseline, markSaved, lastFileRef, hasCopiedCell };
}
