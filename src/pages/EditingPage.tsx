...
import { addActivity } from '../services/cloud';
import { useAuth } from '../context/AuthContext';
import { useToast } from '../hooks/useToast';
import { getPlan } from '../data/plans';
import { PdfGenerationModal } from '../components/PdfGenerationModal';
import { listPdfConfigurations } from '../services/cloud';
...
export function EditingPage({ filename }: { filename: string }) {
  ...
  const { session, raw, config, report, hasCopiedCell } = editor;
  ...
  const [pdfConfigs, setPdfConfigs] = useState<import('../types/cloud').PdfConfiguration[]>([]);
  const [savedCloud, setSavedCloud] = useState<{ project: Project; file: ProjectFile } | null>(stored?.cloud ?? null);
  const { session: authSession } = useAuth();
  const toast = useToast();
  const internalNavigationRef = React.useRef(false);

  useEffect(() => {
    if (stored) setEditingSession(session.state, stored.baseline, lastFile, savedCloud);
  }, [session.state, stored, lastFile, savedCloud]);
  ...
  const handleReplaceWithFile = (file: File) => {
  ...
  const handleSave = () => { if (!authSession) { navigate('/auth/login'); return; } if (!savedCloud || !editor.hasDataChanges) return; if (savedCloud.project.accessRole === 'member') { toast.push('Users have download-only access to this project.','error'); return; } setSaveConfirmOpen(true); };
  const handleAddToProject = () => { if (!authSession) { navigate('/auth/login'); return; } setAddProjectOpen(true); };
  const handlePdf = async () => { if (!authSession) { navigate('/auth/login'); return; } if (savedCloud?.project.id) { try { setPdfConfigs(await listPdfConfigurations(authSession,savedCloud.project.id)); } catch { setPdfConfigs([]); } } setPdfOpen(true); };
  ...
  return <>
    ...
    onSave={savedCloud ? handleSave : undefined}
    saveLabel={'Save changes'}
    canSave={Boolean(savedCloud && editor.hasDataChanges && savedCloud.project.accessRole !== 'member')}
  ...
  />
    ...
    {lastFile && <SaveToProjectModal
      open={saveOpen}
      onClose={() => setSaveOpen(false)}
      document={session.state}
      file={lastFile}
      existing={savedCloud}
      onSaved={(project, file) => { setSavedCloud({ project, file }); editor.markSaved(); setEditingSession(session.state, session.state, lastFile, { project, file }); if (authSession) void addActivity(authSession, { type: 'file_edited', projectId: project.id, fileId: file.id, label: `Worked on “${file.originalName}”` }).catch(() => undefined); }}
    />}

    {lastFile && <SaveToProjectModal open={addProjectOpen} onClose={()=>setAddProjectOpen(false)} document={session.state} file={lastFile} onSaved={(project)=>{setAddProjectOpen(false);toast.push(`Added to ${project.name}`,'success')}} />}

    <PdfGenerationModal open={pdfOpen} onClose={()=>setPdfOpen(false)} report={report} fileName={lastFile?.name || filename} projectId={savedCloud?.project.id} role={savedCloud?.project.accessRole==='member'?'member':savedCloud?.project.accessRole==='editor'?'editor':'owner'} savedConfigs={pdfConfigs} />

    <Modal open={saveConfirmOpen} onClose={()=>setSaveConfirmOpen(false)} title="Save changes" description="Update the saved project file with your current editor changes." size="sm" footer={<div className="flex justify-end gap-2"><button className="modal-cancel-button-lg" onClick={()=>setSaveConfirmOpen(false)}>Cancel</button><button className="btn-primary" onClick={()=>{setSaveConfirmOpen(false);setSaveOpen(true)}}>Continue</button></div>}><p className="text-sm leading-6 text-slate-600">Your current changes will become the next saved version of this project file. The original source file remains preserved.</p></Modal>
