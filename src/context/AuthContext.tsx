...
import { getProfile,updateProfileData } from '../services/cloud';
...
export function AuthProvider({children}:{children:React.ReactNode}){
 ...
 useEffect(()=>{let alive=true;const register=(s:any)=>{if(!s)return;let sessionId='';try{sessionId=localStorage.getItem('docbit_session_id')||'';}catch{};void fetch('/.netlify/functions/session-register',{method:'POST',headers:{Authorization:`Bearer ${s.access_token}`,'Content-Type':'application/json'},body:JSON.stringify({sessionId})}).then(r=>r.json()).then(result=>{if(result?.session?.id){try{localStorage.setItem('docbit_session_id',result.session.id);}catch{}}}).catch(()=>undefined)};(async()=>{try{const {data}=await supabase.auth.getSession();if(alive){await hydrate(data.session);register(data.session);}}finally{if(alive)setLoading(false);}})();const {data:{subscription}}=supabase.auth.onAuthStateChange((event,s)=>{void hydrate(s);if(event==='SIGNED_IN')register(s);});return()=>{alive=false;subscription.unsubscribe()};},[hydrate]);
