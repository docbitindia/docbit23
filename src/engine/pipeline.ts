...
function processReportInternal(raw: RawDataset, config: ReportConfig): ProcessedReport {
  ...
  const normalizedBySource = new Map(normalized.rows.map((r) => [r.sourceIndex, r]));
  const allDataRows = normalized.rows.map((r) => ({ sourceIndex: r.sourceIndex, raw: r.values }));
  ...
  return {
    ...
    quality,
    warnings,
    profile: profileData(meaningful.map((r) => r.raw), schema.columns.length)
  };
}
