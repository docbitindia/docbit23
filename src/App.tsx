...
function RouteContent(){
 ...
 useEffect(()=>{if(loading)return; const protectedPath=['/workspace','/projects','/team','/profile','/settings','/usage-plan'].some(p=>path===p||path.startsWith(`${p}/`)); if(path==='/'&&user&&!deactivatedAccount){navigate('/workspace');return;} if(protectedPath&&(!user||deactivatedAccount))navigate(`/auth/login?redirect=${encodeURIComponent(path)}`)},[path,user,loading]);
 ...
 if(path==='/team') return <TeamMembersPage/>;
 if(path.startsWith('/projects/')) return <ProjectDetailPage/>;
 if(path==='/profile') return <ProfilePage/>;
 if(path==='/settings') return <SettingsPage/>;
 if(path==='/usage-plan') return <UsagePlanPage/>;
