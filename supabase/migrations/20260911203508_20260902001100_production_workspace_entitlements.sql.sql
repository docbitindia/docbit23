...
-- Backfill one default workspace per account.
...
-- Attach existing projects to their owner's default workspace.
update public.projects p
set workspace_id = w.id
from public.workspaces w
where p.workspace_id is null and w.owner_id=p.owner_id and w.slug is not distinct from 'default';

-- For any remaining, use a correlated subquery.
update public.projects p
set workspace_id = (select w.id from public.workspaces w where w.owner_id=p.owner_id order by w.created_at asc limit 1)
where p.workspace_id is null
and exists(select 1 from public.workspaces w where w.owner_id=p.owner_id);

-- Create fallback workspace for any orphaned projects
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM public.projects WHERE workspace_id IS NULL) THEN
    insert into public.workspaces(owner_id,name,description,slug)
    select distinct p.owner_id, 'My Workspace', 'Your default DocBit workspace.', 'default'
...
-- Functions (must be defined before RLS policies that reference them)
...
language sql immutable as $$
  select case p_key
    ...
    when 'processed_files' then case p_plan when 'free' then 10 when 'starter' then 50 when 'pro' then 1000 when 'pro_plus' then 10000 else 10 end
...
begin
 ...
 if p_role='editor' then
   if public.plan_limit(p_plan,'editors')=0 then raise exception 'Your plan does not include editor seats.' using errcode='P3001'; end if;
   if public.plan_limit(p_plan,'editors') is not null and (select count(*) from public.workspace_members where workspace_id=p_workspace_id and role='editor')>=public.plan_limit(p_plan,'editors') then raise exception 'Editor seat limit reached for your % plan.',p_plan using errcode='P3002'; end if;
 elsif lim=0 then raise exception 'Your plan does not include team users.' using errcode='P3003';
 elsif lim is not null and current_count>=lim then raise exception 'User limit reached for your % plan.',p_plan using errcode='P3004'; end if;
...
-- RLS Policies (after all functions are defined)
